...
begin
  ...
  if from > length(str) then begin
    error := 'Пустая строка или нет входных данных';
    result := false;
  end else begin
    p := from;
    ...
    begin
      ...
      if state = 0 then begin
        y0 := getDigit;
        if y0 < 0 then begin
          error := 'Ожидается цифра, встретилось это: '+str[p];
          state := -1;
        end else begin
          state := 1;
        end;
      ...
      if state = 4 then begin
        if not (str[p] = WideChar('-')) then begin
          error :=  'Ожидается тире, встретилось это: '+str[p];
          state := -1;
        end else begin
          state := 5;
        end;
